        ...
        <div v-if="isMapFullscreen" class="fixed inset-0 z-[120] bg-black flex flex-col animate-fade-in pt-safe md:pt-0">
           ...
           <div class="p-8 bg-[#0a0a0a] border-t border-white/10 grid grid-cols-2 gap-8 z-[200]">
              <div>
                 ...
                 <p class="text-4xl font-black italic text-white">{{ Number(selectedActivity.distance || 0).toFixed(1) }} <span class="text-sm text-volt">KM</span></p>
              </div>
              <div class="text-right">
...
const selectActivity = async (act) => {
  ...
  if (act.route && act.route.length > 0) {
    ...
    try {
      ...
      mapInstance = L.map(mapContainer.value, { zoomControl: false });
      ...
      if (Array.isArray(parsedRoute) && parsedRoute.length > 0) {
        polylineInstance = L.polyline(parsedRoute, { color: '#CEFF00', weight: 6 }).addTo(mapInstance);
...
const recenterMap = () => {
  if (mapInstance && polylineInstance) {
    mapInstance.fitBounds(polylineInstance.getBounds(), { padding: [60, 60], animate: true });
  }
};
